-- Survey Tool Database Schema for Supabase - SAFE VERSION
...
-- ========================================
-- 7. ROW LEVEL SECURITY POLICIES (SAFE)
-- ========================================
...
DROP POLICY IF EXISTS "Question templates are publicly readable" ON question_templates;
DROP POLICY IF EXISTS "Public read access to organizational assessments" ON organizational_assessments;
DROP POLICY IF EXISTS "Public insert access to organizational assessments" ON organizational_assessments;
DROP POLICY IF EXISTS "Public update access to organizational assessments" ON organizational_assessments;
DROP POLICY IF EXISTS "Public delete access to organizational assessments" ON organizational_assessments;
DROP POLICY IF EXISTS "Public access to participant responses" ON participant_responses;
DROP POLICY IF EXISTS "Public access to surveys" ON surveys;
DROP POLICY IF EXISTS "Public access to participant sessions" ON participant_sessions;
DROP POLICY IF EXISTS "Public access to data processing records" ON data_processing_records;
...
DROP INDEX IF EXISTS idx_org_assessments_status;
DROP INDEX IF EXISTS idx_org_assessments_consultant;
DROP INDEX IF EXISTS idx_org_assessments_created;
DROP INDEX IF EXISTS idx_participant_responses_assessment;
DROP INDEX IF EXISTS idx_participant_responses_role;
DROP INDEX IF EXISTS idx_participant_responses_department;
DROP INDEX IF EXISTS idx_participant_sessions_survey;
DROP INDEX IF EXISTS idx_participant_sessions_completed;

-- Recreate indexes
CREATE INDEX idx_org_assessments_status ON organizational_assessments(status);
CREATE INDEX idx_org_assessments_consultant ON organizational_assessments(consultant_id);
CREATE INDEX idx_org_assessments_created ON organizational_assessments(created);
CREATE INDEX idx_participant_responses_assessment ON participant_responses(assessment_id);
CREATE INDEX idx_participant_responses_role ON participant_responses(role);
CREATE INDEX idx_participant_responses_department ON participant_responses(department);
CREATE INDEX idx_participant_sessions_survey ON participant_sessions(survey_id);
CREATE INDEX idx_participant_sessions_completed ON participant_sessions(completed_at);

-- ========================================
-- 9. TRIGGERS FOR UPDATED_AT (SAFE)
-- ========================================

-- Drop existing triggers first
DROP TRIGGER IF EXISTS update_organizational_assessments_updated_at ON organizational_assessments;
DROP TRIGGER IF EXISTS update_participant_responses_updated_at ON participant_responses;
DROP TRIGGER IF EXISTS update_surveys_updated_at ON surveys;
DROP TRIGGER IF EXISTS update_participant_sessions_updated_at ON participant_sessions;
...
-- ========================================
-- 10. INITIAL DATA POPULATION (SAFE)
-- ========================================

-- Insert default strategic focus templates (use UPSERT)
...
-- Create/update demo organizational assessment (use UPSERT)
INSERT INTO organizational_assessments (
  ...
  'DEMO-2025-STRATEGY',
  '[
    {"id": "engineering", "name": "Engineering", "managementCode": "DEMO-MGMT-ENG25", "employeeCode": "DEMO-EMP-ENG25"},
    {"id": "sales", "name": "Sales", "managementCode": "DEMO-MGMT-SAL25", "employeeCode": "DEMO-EMP-SAL25"},
    {"id": "marketing", "name": "Marketing", "managementCode": "DEMO-MGMT-MKT25", "employeeCode": "DEMO-EMP-MKT25"},
    {"id": "operations", "name": "Operations", "managementCode": "DEMO-MGMT-OPS25", "employeeCode": "DEMO-EMP-OPS25"}
  ]',
  '[
    {"id": "sa1", "text": "Our organization has a clear, well-communicated strategy", "category": "strategic-clarity", "order": 1},
    {"id": "sa2", "text": "I understand how my role contributes to our strategic objectives", "category": "strategic-clarity", "order": 2},
  ...
  management_responses = EXCLUDED.management_responses,
  employee_responses = EXCLUDED.employee_responses,
